...
switch (toLower _type) do
{
	case "genstore":
	{
		_npc addAction ["<img image='client\icons\store.paa'/> Open GSN General Store", "client\systems\generalStore\loadGenStore.sqf", [], 1, false, false, "", STORE_ACTION_CONDITION];
	};
	case "gunstore":
	{
		_npc addAction ["<img image='client\icons\store.paa'/> Open GSN Gun Store", "client\systems\gunStore\loadgunStore.sqf", [], 1, false, false, "", STORE_ACTION_CONDITION];
	};
	case "vehstore":
	{
		_npc addAction ["<img image='client\icons\store.paa'/> Open GSN Vehicle Store", "client\systems\vehicleStore\loadVehicleStore.sqf", [], 1, false, false, "", STORE_ACTION_CONDITION];
	};
};
